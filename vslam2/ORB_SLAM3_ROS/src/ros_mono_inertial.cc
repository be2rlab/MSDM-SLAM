...
int main(int argc, char** argv) {
  ...
  ros::Subscriber sub_imu =
      n.subscribe("/imu", 100, &ImuGrabber::GrabImu, &imugb);
  ros::Subscriber sub_img0 =
      n.subscribe("/camera/image_raw", 10, &ImageGrabber::GrabImage, &igb);
...
void ImageGrabber::SyncWithImu() {
  while (1) {
    ...
    // double tIm = 0;
    if (!img0Buf.empty()) {
      const double tIm = img0Buf.front()->header.stamp.toSec();
      ...
      ROS_WARN_STREAM("num imus: " << vImuMeas.size() << " " << tIm);
