...
int main(int argc, char** argv) {
  ...
  if (argc < 5) {
    cerr << endl
         << "Usage: ./rgbd_tum path_to_vocabulary path_to_settings "
            "path_to_sequence path_to_association (file_name)"
         << endl;
    return 1;
  }
  const int num_seq = (argc - 3) / 2;

  if (((argc - 3) % 2) == 1) bFilename = true;
  ...
  for (int seq = 0; seq < num_seq; seq++) {
    ...
    if (vstrImageFilenamesRGB[seq].empty()) {
      ...
      return 1;
    } else if (vstrImageFilenamesD[seq].size() !=
               vstrImageFilenamesRGB[seq].size()) {
      cerr << endl << "Different number of images for rgb and depth." << endl;
      return 1;
    }
    std::cout << "Loaded sequence " << seq
              << "! Num of images: " << nImages[seq] << std::endl;
  ...
  for (int seq = 0; seq < num_seq; seq++) {
    ...
    for (int ni = 0; ni < nImages[seq]; ni++) {
      ...
      imRGB = cv::imread(
          string(argv[3 + seq * 2]) + "/" + vstrImageFilenamesRGB[seq][ni],
          cv::IMREAD_UNCHANGED);
      imD = cv::imread(
          string(argv[3 + seq * 2]) + "/" + vstrImageFilenamesD[seq][ni],
          cv::IMREAD_UNCHANGED);
