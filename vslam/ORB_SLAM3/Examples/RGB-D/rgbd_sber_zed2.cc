...
int main(int argc, char** argv) {
  ...
  if (argc < 4) {
    cerr << endl
         << "Usage: ./rgbd_tum path_to_vocabulary path_to_settings "
            "path_to_sequence (file_name)"
         << endl;
    return 1;
  }

  if (argc == 5) bFilename = true;
  ...
  for (int ni = 0; ni < nImages; ni++) {
    ...
    imRGB = cv::imread(
        string(argv[3]) + "/left_resize/" + vstrImageFilenamesRGB[ni],
        cv::IMREAD_UNCHANGED);
    imD =
        cv::imread(string(argv[3]) + "/depth_resize/" + vstrImageFilenamesD[ni],
                   cv::IMREAD_UNCHANGED);
